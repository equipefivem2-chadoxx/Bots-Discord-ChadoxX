...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Envoie l\'embed du règlement officiel du serveur.')
        // Sécurité : Seul le staff avec la permission Administrateur ou Gérer le serveur peut l'utiliser
        .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
        
    async execute(interaction) {
        // Le salon cible que tu as défini
        ...
        // --- DESIGN PREMIUM DU RÈGLEMENT ---
        const goldColor = '#d4af37'; // Couleur or pour le côté officiel
        ...
        try {
            await targetChannel.send({ embeds: [reglementEmbed] });
            // Réponse éphémère pour confirmer sans polluer le salon
            await interaction.reply({ content: `✅ Le règlement a été publié avec succès dans <#${targetChannelId}> !`, ephemeral: true });
        } catch (error) {
            console.error(error);
            await interaction.reply({ content: "❌ Une erreur s'est produite lors de l'envoi. Vérifie que le bot a la permission d'écrire et d'intégrer des liens dans ce salon.", ephemeral: true });
        }
    }
};